...
def faz_matriz(f, fp, x1, y1, x2, y2, N, eps, maxiter):
    ...
    for linha in range (m):
        ...
        for coluna in range(n):
            I[linha].append(newton(f,fp,complex(x1+coluna*delta,y2-linha*delta),eps,maxiter)[0])
            R[linha].append(newton(f,fp,complex(x1+coluna*delta,y2-linha*delta),eps,maxiter)[1])
