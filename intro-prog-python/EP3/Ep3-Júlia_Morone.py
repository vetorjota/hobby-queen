"""
  ...
  https://www.ime.usp.br/~kellyrb/mac2166_2017/arquivos/mac2166_aula19.html
...
def faz_matriz(f, fp, x1, y1, x2, y2, N, eps, maxiter):
    ...
    for linha in range (m):
        ...
        for coluna in range(n):
            I[linha].append(newton(f,fp,complex(x1+linha*delta,y2-coluna*delta),eps,maxiter)[0])
            R[linha].append(newton(f,fp,complex(x1+linha*delta,y2-coluna*delta),eps,maxiter)[1])
...
def determina_cores(I, R, ro):
    ...
    """
    # Veja como obter novas cores:
    '''a = nova_cor()
    b = nova_cor()
    print(a, b, nova_cor())'''
    # Remova o código acima antes de implementar sua função.


    #para cada ponto atribuir uma cor
    #cores iguais para pontos que induzem a uma mesma raiz
    #cada ponto da imagem é um ponto da grade(matriz)
    #para criar os grupos, todos os valores seriam as possíveis raízes, no espaço S
    #queremos encontrar valores tais q quaisquer 2 pontos/centros estejam suficientemente longe (ro) um do outro
    #a partir deles, vamos associar cada ponto de S a um centro
    ...
    for i in range (len(I)):
        for j in range (len(I[0])):
            '''
            if I[i][j] != -1 and not achou:
                centros.append (R[i][j])
                achou = True
            '''

            esta_perto_de_um_centro = False

            if I[i][j] != -1:
                for centro in centros:
                    if abs (R[i][j] - centro) < ro:
                        esta_perto_de_um_centro = True

                if not esta_perto_de_um_centro :
                    centros.append (R[i][j])
    ...
    for i in range (len(I)):
        ...
        for j in range (len(I[0])):
            if I[i][j] != -1:
                #associar cada (i,j) a um centro R[i][j]
                #encontrar centros
                S.append (R[i][j])
    ...
    if (x-i)**2 + (y-j)**2 < ro **2   #está na região círculo de centro i,j e raio ro
...
def calcula_sombras(I):
    ...
    lista_s = (N+1) * [0]
    soma_a = 0
    lista_s[0] = 1 - soma_a/S
    for k in range (1,N+1):
        soma_a += lista_a[k-1]
        lista_s[k] = 1 - soma_a/S
...
def grava_imagem(I, C, s, alfa, nome_arquivo):
    ...
    arquivo.write('P3\n')
    arquivo.write(str(len(C)) +' '+ str(len(C[0]))+'\n')
    arquivo.write('255\n')
    
    
    for i in range (len(I)):
        for j in range (len(I[0])):           #desse jeito imprime a transposta da matriz da outra função
            for k in (C[i][j]):
                arquivo.write(str(int(1 - alfa + alfa* s[(I[i][j])] * k))+' ')
