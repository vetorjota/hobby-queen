...
def deslocar_coluna ( tabuleiro, i ):
    ...
    '''
    colunai = []    #lista para armazenar elementos não "vazios" da coluna com espaços em cima
    for n in range (len(tabuleiro)):
        colunai.append (" ")
        
    linha = (len(tabuleiro)) - 1
    indice = 0
    while linha > -1 and indice < (len(colunai)):
        if tabuleiro[linha][i] != " ":
            colunai[indice] = (tabuleiro[linha][i])
            indice += 1
            linha -= 1
        else:
            linha -= 1
    ...
    for lin in range (len(tabuleiro)-1,-1,-1):
        for k in range (len(colunai)):
            tabuleiro[lin][i] = colunai[k]
